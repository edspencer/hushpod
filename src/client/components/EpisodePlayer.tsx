...
export function EpisodePlayer({
  ...
  const [speed, setSpeed] = useState<number>(() => {
    const saved = playback.load(episodeId);
    return saved && SPEEDS.includes(saved.speed as (typeof SPEEDS)[number])
      ? saved.speed
      : 1;
  });
  ...
  const handleLoadedMetadata = useCallback(() => {
    ...
    if (saved && saved.version === version && saved.position > 0) {
      const clamped = Math.min(
        saved.position,
        (audio.duration || saved.position) - 0.25,
      );
      if (clamped > 0) {
    ...
    return (
      ...
      >
        <p className="text-sm text-muted">
          No audio available yet. Run processing to generate the cleaned
          episode.
        </p>
      </div>
    );
      ...
      {showMarkers && (
        <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-border pt-3 text-[11px] text-muted">
          <span className="inline-flex items-center gap-1">
            <RotateCcw className="h-3 w-3" /> Markers show removed segments on
