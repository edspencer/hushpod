    ...
    <div className="space-y-6">
      ...
      <header className="flex flex-col gap-5 sm:flex-row sm:items-start">
        ...
        <div className="min-w-0 flex-1 space-y-3">
          ...
          {show.description && (
            <p className="line-clamp-3 max-w-2xl text-sm text-muted">
              {show.description}
            </p>
          )}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {checkShow.isSuccess && checkResult != null && (
          <span className="inline-flex items-center gap-1.5 text-sm text-success">
            ...
            {checkResult === 0
              ? 'No new episodes'
              : `Found ${checkResult} new ${
                  checkResult === 1 ? 'episode' : 'episodes'
                }`}
          </span>
        )}
        {checkShow.isError && (
        ...
        <label className="flex cursor-pointer items-start gap-2.5 rounded-md border border-border bg-surface-2/40 p-3">
          ...
          <span className="text-sm">
            ...
            <span className="mt-0.5 block text-xs text-muted">
              Permanently remove processed audio from disk. This cannot be
              undone.
            </span>
          </span>
        </label>
