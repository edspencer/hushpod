...
export const api = {
  ...
  getShow: (id: number): Promise<ShowWithEpisodes> =>
    request<ShowWithEpisodes>(`/api/shows/${id}`),

  updateShow: (
    id: number,
    patch: Partial<
      Pick<Show, 'isActive' | 'episodeLimit' | 'removeAds' | 'removePromos'>
    >,
  ): Promise<Show> =>
  ...
  deleteShow: (id: number, deleteFiles = false): Promise<{ ok: true }> =>
    request<{ ok: true }>(
      `/api/shows/${id}${deleteFiles ? '?deleteFiles=true' : ''}`,
      { method: 'DELETE' },
    ),
  ...
  getEpisode: (id: number): Promise<EpisodeDetail> =>
  ...
  listShowAds: (showId: number): Promise<Ad[]> =>
    request<Ad[]>(`/api/shows/${showId}/ads`),
...
export function useShow(
  id: number | undefined,
): UseQueryResult<ShowWithEpisodes, Error> {
  return useQuery({
...
export function useEpisode(
  id: number | undefined,
): UseQueryResult<EpisodeDetail, Error> {
  return useQuery({
...
): UseQueryResult<SystemStatus, Error> {
  return useQuery({
...
export function useAddShow(): UseMutationResult<
  AddShowResponse,
  Error,
  string
> {
...
export function useCheckShow(): UseMutationResult<
  { discovered: number },
  Error,
  number
> {
...
export function useProcessEpisode(): UseMutationResult<
  ProcessResponse,
  Error,
  number
> {
...
export function useReprocessEpisode(): UseMutationResult<
  ProcessResponse,
  Error,
  number
> {
